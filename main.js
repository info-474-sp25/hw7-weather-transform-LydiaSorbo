...
d3.csv("weather.csv").then(data => {
    ...
    */
    ...
    const groupedData1 = d3.groups(filteredData1, d => d.city, d => d.year).map(([city, years]) => ({
        city, 
    ...
    */
    const flattenedData = groupedData1.flatMap(({ city, values}) => values.map(({ year, avgPrecip }) => ({
            year, 
            avgPrecip,
            city
        }))
    ...
    */
    const groupedData2 = d3.groups(filteredData2, d => d.month).map(([month, entries]) => ({
        month, 
    ...
    */
    const pivotedData = groupedData2.flatMap(({ month, actualPrecip, avgPrecip, recordPrecip }) => [
        { month, precipitation: actualPrecip, meansurement: "Actual" },
        { month, precipiatation: avgPrecip, measurement: "Average" },
        { month, precipiatation: recordPrecip, measurement: "Record"}
    ]);// Your code here!
